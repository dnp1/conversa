...
func TestRoomController_DeleteRoom(t *testing.T) {
    ...
    cases := []Case{
        ...
        {//couldn't delete
            func() *gin.Engine {
                ...
                r.EXPECT().Delete(sessionData.Username, "golang").Return(room.ErrCouldNotDelete)
                rb := server.RouterBuilder{
                    Session:s,
                    Room:r,
                }
        ...
        {//Everything ok
            func() *gin.Engine {
                ...
                sessionData := &session.Data{Username:"dnp1"}
                s.EXPECT().Retrieve(tokens[4].String()).Return(sessionData, nil)
                r.EXPECT().Delete(sessionData.Username, "golang").Return(nil)
                rb := server.RouterBuilder{
                    Session:s,
                    Room:r,
                }
    ...
    for i, c := range cases {
        url := fmt.Sprintf("/users/%s/rooms/%s", c.user, "golang")
        req, err := http.NewRequest("DELETE", url, c.body)
        if tokens[i] != nil {
            req.AddCookie(&http.Cookie{
                Name: server.TokenCookieName,
                Value: tokens[i].String(),
